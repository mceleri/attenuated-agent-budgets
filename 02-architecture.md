...
3. **Lineage Validation & IDOR Prevention**: The Provider validates that the presented `revocation_secret` matches the designated `checkout_id`. Once authenticated, the target `revocation_id` is appended to the ledger's revoked block registry for that account.
