# Attenuated Agent Budgets: Macaroon-Based Sub-Budget Delegation for Multi-Agent Swarms via Merchant-of-Record-Funded Payments
...
Instead, payment can be decoupled from access delegation: a single, human-authorized transaction is processed through a MoR, and a "Master Macaroon" is issued upon successful settlement.
This macaroon acts purely as a locally verifiable, cryptographically attenuable access-control credential, independent of any specific M2M payment protocol.

A primary orchestrator agent can attenuate the Master Macaroon offline into strict sub-budgets, distributing them to autonomous sub-agents without further interaction with the orchestrator.
...
- sharing macaroon root-key material across multiple services from the same provider introduces a security trade-off between operational simplicity and blast-radius containment.
